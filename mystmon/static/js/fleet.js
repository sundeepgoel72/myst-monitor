    ...
    try {
        ...
        allNodes = allNodes.map(node => {
            ...
            return {
                ...
                running: normalizeBoolean(node.running),
                status: MystMonUtils.getNodeStatus(normalizeKnown(node.online), normalizeBoolean(node.running)).value,
                quality: normalizeNumber(node.quality),
                earnings_24h: normalizeNumber(node.earnings_total),
                uptime_24h: normalizeNumber(
                    node.uptime_minutes_24h !== null && node.uptime_minutes_24h !== undefined
                        ? node.uptime_minutes_24h
                        : (node.uptime_seconds !== null && node.uptime_seconds !== undefined ? node.uptime_seconds / 60 : null)
                ),
                restarts: normalizeNumber(node.restart_count),
                api_enabled: normalizeBoolean(node.api_enabled),
                api_schema_available: normalizeBoolean(node.api_schema_available),
                api_nat_type: node.api_nat_type || node.nat_type || '',
                api_public_ip: node.api_public_ip || node.public_ip || node.local_ip || '',
                api_location: formatLocation(node),
                api_services_total: normalizeNumber(node.api_services_count),
                api_services_running: normalizeNumber(node.api_services_running),
                api_sessions_1d: normalizeNumber(node.api_sessions_1d),
                api_sessions_7d: normalizeNumber(node.api_sessions_7d),
                last_seen: node.collected_at,
                local_match: normalizeBoolean(node.local_match),
...
function filterTable() {
    ...
    filteredNodes = allNodes.filter(node => {
        ...
        if (status) {
            ...
            if (status === 'offline' && node.status !== 'offline') return false;
...
function renderTable() {
    ...
    tbody.innerHTML = pageNodes.map(node => {
        ...
        const qualityKnown = node.quality_known === true;
        ...
        const qualityText = quality !== null && quality !== undefined ? Math.round(quality) : (qualityKnown ? '-' : 'Unknown');
        const status = MystMonUtils.getNodeStatus(node.online, node.running);
        
        const earnings24h = node.earnings_24h;
        const uptime24h = node.uptime_24h;
        const restarts = node.restarts !== null && node.restarts !== undefined ? node.restarts : 0;
        const apiStatus = node.api_enabled === true ? 'Up' : (node.api_schema_available === false ? 'Unsupported' : 'Unavailable');
        const apiNat = node.api_nat_type || '-';
        const apiPublicIp = node.api_public_ip || '-';
        const apiLocation = node.api_location || '-';
        const servicesSummary = node.api_services_running !== null && node.api_services_total !== null
            ? `${node.api_services_running}/${node.api_services_total}`
            : '-';
        const sessionsSummary = [node.api_sessions_1d, node.api_sessions_7d].map(v => v !== null && v !== undefined ? v : '-').join(' / ');
        ...
        return `
            <tr class="${isSelected ? 'selected bg-primary-50 dark:bg-primary-900/30' : 'hover:bg-gray-50 dark:hover:bg-gray-800'}" data-node-key="${MystMonUtils.escapeHtml(node.node_key)}" onclick="toggleRowSelection('${MystMonUtils.escapeHtml(node.node_key)}', event)">
                ...
                <td class="px-4 py-3 text-sm text-gray-900 dark:text-white">${MystMonUtils.escapeHtml(apiNat)}</td>
                <td class="px-4 py-3 text-sm text-gray-900 dark:text-white">${MystMonUtils.escapeHtml(apiPublicIp)}</td>
                <td class="px-4 py-3 text-sm text-gray-900 dark:text-white">${MystMonUtils.escapeHtml(apiLocation)}</td>
                <td class="px-4 py-3 text-sm text-gray-900 dark:text-white">${MystMonUtils.escapeHtml(servicesSummary)}</td>
                <td class="px-4 py-3 text-sm text-gray-900 dark:text-white">${MystMonUtils.escapeHtml(sessionsSummary)}</td>
                <td class="px-4 py-3 text-sm text-gray-900 dark:text-white">${apiStatus}</td>
                <td class="px-4 py-3">
                    <div class="flex items-center space-x-2">
                        <span class="w-2.5 h-2.5 rounded-full ${status.dotClass}"></span>
                        <span class="text-sm font-medium ${status.textClass}">${status.label}</span>
                    </div>
                ...
                <td class="px-4 py-3 text-sm text-gray-900 dark:text-white">${node.earnings_known === true ? MystMonUtils.formatEarnings(earnings24h) : 'Unknown'}</td>
                <td class="px-4 py-3 text-sm text-gray-900 dark:text-white">${node.uptime_known === true ? MystMonUtils.formatUptimeMinutes(uptime24h) : 'Unknown'}</td>
                <td class="px-4 py-3 text-sm text-gray-900 dark:text-white">${restarts}</td>
...
function formatLocation(node) {
    const parts = [node.api_location_city, node.api_location_country].filter(Boolean);
...
function exportToCsv(nodes, filename) {
    ...
    const rows = nodes.map(node => {
        const quality = node.quality !== null && node.quality !== undefined ? Math.round(node.quality) : '';
        const earnings24h = node.earnings_24h !== null && node.earnings_24h !== undefined ? node.earnings_24h : '';
        const uptime24h = node.uptime_24h !== null && node.uptime_24h !== undefined ? node.uptime_24h : '';
        const restarts = (node.restarts !== null && node.restarts !== undefined) ? node.restarts : 0;
        const online = (node.online !== null && node.online !== undefined) ? (node.online ? 'Online' : 'Offline') : '';
