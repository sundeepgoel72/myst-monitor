    ...
    try {
        ...
        if (tbody) {
            tbody.innerHTML = Object.entries(collectors).map(([name, info]) => {
                const badgeClass = info.status === 'ok'
        ...
        if (pre) {
            const collectorConfig = {
                ...
                snmp: config.snmp,
                mystnodes_accounts: config.mystnodes_accounts || [],
            };
...
async function loadPortalConfig() {
    try {
        const config = await MystMonApi.config();
        const portalAccounts = Array.isArray(config.mystnodes_accounts) ? config.mystnodes_accounts : [];
        const portalConfig = portalAccounts[0] || {};
        
        const pre = document.getElementById('portal-config-json');
        if (pre) {
            // Redact sensitive fields
            const safeConfig = portalAccounts.map(account => {
                const copy = { ...account };
                if (copy.email_env) copy.email_env = '***';
                if (copy.password_env) copy.password_env = '***';
                return copy;
            });
